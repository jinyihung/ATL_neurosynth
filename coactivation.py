...
def coactivation_contrast(dataset, infile, regions=None, target_thresh=0.01, 
    ...
    if regions == None:
        regions = np.round(np.unique(image.get_data()))[1:]
    ...
    result = []
    for ix, ma in enumerate(meta_analyses):
        # print(ix)
        tmp = nib.nifti1.Nifti1Image(dataset.masker.unmask( ma.images[stat]), affine, dataset.masker.get_header())
        result += [tmp]
    return result
